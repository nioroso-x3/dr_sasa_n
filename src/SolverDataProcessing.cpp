...
PrintSplitAsaAtom(vector<atom_struct>& pdb,
  ...
  const char* formatS = "%s\t%s\t%d%s\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f";
...
/*  vector<atom_struct*> prot;
  ...
  auto printline = [&](atom_struct* atom){
    ...
    lig_polar_ASA = NAN;
    lig_hyd_ASA = NAN;
    if((find(amino_acids.begin(),amino_acids.end(),atom->RESN) != amino_acids.end()) && (atom->MOL_TYPE == "PROTEIN")){
    ...
    else if( (find(bases.begin(),bases.end(),atom->RESN) != bases.end()) && (atom->MOL_TYPE=="DNA" || atom->MOL_TYPE =="RNA" )){
      string resname2;
    ...
    outf << atom->NAME << "\t";
    char buffer[8192];
    snprintf(buffer,8192,formatS,atom->RESN.c_str(),atom->CHAIN.c_str(),atom->RESI,atom->iCODE.c_str(),atom->ID,total_ASA,bb_ASA,sc_ASA,majorgroove_ASA,minorgroove_ASA,nogroove_ASA,polar_ASA,polar_bb_ASA,polar_sc_ASA,polar_majorgroove_ASA,polar_minorgroove_ASA,polar_nogroove_ASA,hyd_ASA,hyd_bb_ASA,hyd_sc_ASA,hyd_majorgroove_ASA,hyd_minorgroove_ASA,hyd_nogroove_ASA,lig_ASA,lig_polar_ASA,lig_hyd_ASA,atom->COORDS[0],atom->COORDS[1],atom->COORDS[2]);
